...
namespace Boss {
	public class TraceSkill : MonoBehaviour, SkillState {
		...
		void OnDrawGizmos() {
			...
			Gizmos.DrawLine(transform.position, transform.position + Vector3.up +transform.forward * 5f);
		}
		...
		public void enter (BasicBoss boss) {
			...
			moveBehavior = new MoveBehavior(boss,  self.maxAttackVelocity, delegate {
				if (Physics.Linecast(transform.position, transform.position + Vector3.up + transform.forward * 5f, self.playerLayer)) {
					self.changeState(gameObject.AddComponent<AttackSkill>());
				}
			});
